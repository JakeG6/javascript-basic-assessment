...
// remove 'flash' from the array and store him in a variable called secondHero
	var secondHero = heroes.splice(heroes.length -2, 1).toString();
...
function doubleTheFun(param) {
	if (Number(param)) {return param*2;}
...
function getValueOfProperty(obj, value) {
	for (prop in obj) {
		if (prop === value) {
			return obj[prop];
		}
...
//to have a property called lovesCode with a value of 10.  If their name is Brack
//set lovesCode to 0.  otherwise set lovesCode to 5.
...
// #16 Create a function called callerBack that takes in a function (holla) and
// a string parameter(back) and invokes it(holla) with the argument string(back) + ' back'."
